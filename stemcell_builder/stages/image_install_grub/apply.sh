...
GRUB_CMDLINE_LINUX="vconsole.keymap=us net.ifnames=0 crashkernel=auto selinux=0 plymouth.enable=0 console=tty0 earlyprintk=ttyS0 rootdelay=300 audit=1"
EOF
...
title ${os_name} (${kernel_version})
  root (hd0,0)
  kernel /boot/vmlinuz-${kernel_version} ro root=UUID=${uuid} selinux=0 cgroup_enable=memory swapaccount=1 console=tty0 earlyprintk=ttyS0 rootdelay=300 ipv6.disable=1 audit=1
...
title ${os_name} (${kernel_version})
  root (hd0,0)
  kernel /boot/vmlinuz-${kernel_version} ro root=UUID=${uuid} net.ifnames=0 plymouth.enable=0 selinux=0 console=tty0 earlyprintk=ttyS0 rootdelay=300 ipv6.disable=1 audit=1
  initrd /boot/${initrd_file}
GRUB_CONF
